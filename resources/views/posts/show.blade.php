...
{{--                    @endif--}}
            ...
            @auth
                @if($post->user_id === auth()->user()->id)
                    <form action="{{route('posts.destroy',$post)}}" method="POST">
                        @method('DELETE')
                        @csrf
                        <input type="submit" value="{{__('Delete post')}}"
        ...
        <div class="md:w-1/2 p-5">
            <div class="shadow bg-white p-5 mb-5">
                @auth
                    <p class="text-xl font-bold text-center mb-4">{{__('Add Comment')}}</p>
                    @if(session('message'))
                    ...
                    <form action="{{route('comments.store',[$user,$post])}}" method="POST">
                        ...
                        <textarea id="comment" name="comment" placeholder="{{__('write some')}}"
                ...
                <div class="bg-white shadow mb-5 max-h-96 overflow-y-scroll mt-10">
                    @if($post->comments->count())
                        ...
                        @endforeach
                    @else
                        <p class="p-10 text-center">{{__('None Comments yet')}}</p>
                    @endif
                </div>
            </div>
        </div>
